...
document.getElementById("summarise-button").addEventListener("click", async () => {
...
// 3. Read the selected summary type from the dropdown
  const summmaryType = document.getElementById("summary-type").va
  ...
  try{
    ...
    resultDiv.innerText = `Error:  :${err.message || "Failed to generate summary"}`;

  }
  }); // <-- closes chrome.storage.sync.get callback
}); // <-- closes main click handler
...
async function getGeminiSummary(text, summaryType, apiKey){
  ...
  // WE ARE sending request to gem8ini api with key and prompt
   ...
   return (
    ...
    throw new Error("Failed to get summary from Gemini API. " + error.message);
  }
}
